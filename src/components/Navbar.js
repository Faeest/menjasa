...
import { Box, Button, ButtonGroup, Container, FormLabel, IconButton, Tooltip, Image, Input, InputGroup, Spinner, useColorMode, Link, textDecoration, Menu, MenuButton, MenuList, MenuItem, useDisclosure, Fade, Collapse } from "@chakra-ui/react";
    ...
    return (
        <Box w={"full"} position={"sticky"} alignItems={"center"} justifyContent={"space-around"} display={"flex"} h={"70px"} shadow={"md"} bg={"palette.white"} _dark={{ bg: "palette.dark" }}>
            ...
            <ul href="/about-us" css={css(`list-style:none;display:flex;gap:3rem;`)}>
                ...
                <li className="drophover" onMouseEnter={()=>!isOpen && onToggle()} onMouseLeave={()=>isOpen && onToggle()}>
                    <NavbarLink href="/">FAQ</NavbarLink>
                    <Collapse in={isOpen} animateOpacity>
                        <Box fontWeight={500} className="menu" borderRadius={"base"} shadow={"lg"} color={colorMode == "light"?"palette.light" : "palette.dark"} bg={colorMode == "light"?"palette.black" : "palette.light"}>
                            <Link href="/" _hover={{textDecoration:"none"}}>Test</Link>
                        </Box>
                    </Collapse>
                </li>
                <li>
                    <NavbarLink href="/">Job</NavbarLink>
                </li>
                <li>
                    <NavbarLink href="/">Talent Finder</NavbarLink>
                </li>
            </ul>
